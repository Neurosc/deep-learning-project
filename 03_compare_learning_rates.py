"""
03_compare_learning_rates.py
============================
Small 250 Hz benchmark to compare learning rates before running the expensive
1000 Hz experiment.
...
while keeping the following fixed:
    - same 250 Hz EEG
    - same residual decoder
    - fixed InfoNCE temperature = 0.07
    - same weight decay
    - same batch size
    - same validation split for a given seed
    - same representative ANN layers and EEG windows

This does NOT rerun EEG preprocessing or ANN feature extraction.
...
Early stopping:
    Stops a run when validation loss has not improved for 8 consecutive epochs.

Outputs:
    ~/things_eeg/results/learning_rate_comparison_sub-01.csv
    ~/things_eeg/results/learning_rate_comparison_epochs_sub-01.csv
    ~/things_eeg/results/learning_rate_comparison_paired_sub-01.csv
"""
...
SUMMARY_CSV = os.path.join(
    RES_DIR,
    "learning_rate_comparison_sub-01.csv",
)

EPOCH_CSV = os.path.join(
    RES_DIR,
    "learning_rate_comparison_epochs_sub-01.csv",
)

PAIRED_CSV = os.path.join(
    RES_DIR,
    "learning_rate_comparison_paired_sub-01.csv",
)
...
class ResidualDecoder(nn.Module):
    """
    Same residual projection structure used in the main alignment pipeline.

    Temperature is NOT learned here because the purpose of this diagnostic
    is to isolate the effect of learning rate.
    """
...
# ---------------------------------------------------------------------------
# Train one target x window x seed x learning-rate condition
# ---------------------------------------------------------------------------
def train_one(
    train_eeg,
    train_target,
    ...
    # Re-seed every condition so LR comparisons use the same split and
    # initialization sequence for the same seed.
    ...
    validation_eeg, validation_target = (
        to_batch(
    ...
    test_eeg_tensor, test_target_tensor = (
        to_batch(
            test_eeg,
            test_target,
            window,
    ...
    # Same-size training probe as validation.
    training_probe_indices = (
        training_indices[:n_validation]
    )

    training_probe_eeg, training_probe_target = (
        to_batch(
    ...
    for epoch in range(
        ...
        trace.append(
            {
                "epoch": epoch,
                "learning_rate_condition": learning_rate,
...
def main():
    ...
    # ---------------------------------------------------------------
    # Save raw summaries and epoch traces
    # ---------------------------------------------------------------
    ...
    # ---------------------------------------------------------------
    # Paired 1e-4 minus 1e-3 comparison
    # ---------------------------------------------------------------
    index_columns = [
        "target",
        "window",
    ...
    lr_1e3 = lr_1e3.rename(
        columns={
            column: f"{column}_1e-3"
            for column in value_columns
        }
    )

    lr_1e4 = lr_1e4.rename(
        columns={
            column: f"{column}_1e-4"
            for column in value_columns
        }
    )
    ...
    paired[
        "delta_best_val_1e-4_minus_1e-3"
    ] = (
        paired["best_val_loss_1e-4"]
        - paired["best_val_loss_1e-3"]
    )

    paired[
        "delta_test_loss_1e-4_minus_1e-3"
    ] = (
        paired["test_loss_at_best_1e-4"]
        - paired["test_loss_at_best_1e-3"]
    )

    paired[
        "delta_test_top1_1e-4_minus_1e-3"
    ] = (
        paired["test_top1_at_best_1e-4"]
        - paired["test_top1_at_best_1e-3"]
    )

    paired[
        "delta_gap_at_best_1e-4_minus_1e-3"
    ] = (
        paired["gap_at_best_1e-4"]
        - paired["gap_at_best_1e-3"]
    )
    ...
    # ---------------------------------------------------------------
    print(
        "\n=== Mean performance by learning rate ===",
        flush=True,
    )
    ...
    print(
        "negative delta test loss = 1e-4 better",
        flush=True,
    )

    print(
        "positive delta test top1 = 1e-4 better",
        flush=True,
    )

    print(
        "negative delta gap = 1e-4 smaller gap",
        flush=True,
    )
